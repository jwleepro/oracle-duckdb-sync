...
## 10. 상태/메타데이터 관리
- [x] **TEST-140**: sync_state 저장·로드
- [x] **TEST-141**: 스키마/매핑 설정 버전 관리
- [x] **TEST-142**: 실패 시 상태 롤백·재시작 가능
...
### 11.1 진행 상황 콜백 및 제어
- [x] **TEST-150**: SyncWorker 스레드 시작·종료·상태 확인
- [x] **TEST-151**: 진행 상황 콜백 호출 및 큐 전달
- [ ] **TEST-152**: 일시정지 이벤트로 동기화 중단 및 재개
- [ ] **TEST-153**: 취소 이벤트로 동기화 즉시 중단

### 11.2 UI 대시보드
- [ ] **TEST-160**: 세션 상태 초기화 및 관리
- [ ] **TEST-161**: 진행 메시지 폴링 및 상태 업데이트
- [ ] **TEST-162**: 프로그레스 바 렌더링 (행 수 기준)
- [ ] **TEST-163**: 통계 표시 (처리 속도, 경과 시간)
- [ ] **TEST-164**: 제어 버튼 상태 전환 (시작/일시정지/재개/취소)

### 11.3 실시간 로그 스트리밍
- [ ] **TEST-170**: 로그 메시지 버퍼 관리 (최근 500개)
- [ ] **TEST-171**: 실시간 로그 UI 표시
- [ ] **TEST-172**: 로그 레벨별 색상 구분 (INFO/WARN/ERROR)

### 11.4 오류 처리
- [ ] **TEST-180**: 오류 발생 시 즉시 캡처 및 큐 전달
- [ ] **TEST-181**: 오류 메시지 UI 표시
- [ ] **TEST-182**: 오류 로그 파일 기록
- [ ] **TEST-183**: 오류 패널 expander로 traceback 표시

### 11.5 통합 테스트
- [ ] **TEST-190**: 전체 동기화 진행 상황 E2E 테스트
- [ ] **TEST-191**: 일시정지/재개 E2E 테스트
- [ ] **TEST-192**: 취소 E2E 테스트
- [ ] **TEST-193**: 오류 발생 시 UI 표시 E2E 테스트
