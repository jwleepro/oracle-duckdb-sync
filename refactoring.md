...
**위치:**
- `sync_engine.py:137` - `batch_size=10000`
- `sync_engine.py:156` - `batch_size=10000`
- `sync_engine.py:160` - `batch_size=10000`
- `sync_engine.py:207` - `batch_size=10000`
- `oracle_source.py:147` - `batch_size=1000`

**문제:** 매직 넘버 `10000`이 중앙화된 설정 없이 7곳 이상에 나타납니다.
...
```python
# sync_engine.py
def full_sync(self, oracle_table: str, duckdb_table: str,
              batch_size: int = None, max_duration: int = None):
    batch_size = batch_size or self.config.sync_batch_size
    max_duration = max_duration or self.config.sync_max_duration_seconds
```

**예상 효과:** 15개 이상의 하드코딩된 값 중앙화, 설정 가능성 향상.
...
**위치:**
- `sync_engine.py:339` - `"sync_state.json"`
- `sync_engine.py:362` - `"schema_mappings.json"`
- `sync_engine.py:511` - `"sync_progress.json"`
- 10곳 이상에서 여러 참조
...
### 4.3 하드코딩된 시간 제한 및 임계값

**위치:**
- `sync_engine.py:262` - `max_duration=3600` (1시간)
- `sync_engine.py:82` - `row_limit=100000`
- `app.py:85-87` - 테스트 행 제한 범위 10000-100000
- `visualization.py:18` - 패딩 퍼센트 `0.05`
- `data_converter.py` - 타입 감지 임계값 `0.9`

**문제:** 코드 변경 없이 이러한 값들을 설정할 방법이 없습니다.

**권장사항:** 모두 중앙화된 Config 클래스에 추가.
